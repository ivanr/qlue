...
- Security:
  ...
  - Automatic context-aware output encoding in Velocity templates, on by default. It encodes body
    ...
    attributes with an origin filter on the ones that fetch code, and refuses to write into
    JavaScript and CSS contexts at all. An attribute name it does not recognise is **suppressed**,
    not encoded. It is **not** a complete XSS defence — see
    [qlue_context_aware_encoding.md](qlue_context_aware_encoding.md) for what it does not cover, and
    for how to widen the defaults when they are wrong for your page.
...
and can only be turned off by application code (`VelocityViewFactory.setAutoEscaping(false)`).

The subject has a document of its own:
[qlue_context_aware_encoding.md](qlue_context_aware_encoding.md). It covers what Canoe encodes, what
it suppresses to the empty string (including any attribute name it does not recognise), what markup
it rejects outright, how to widen the defaults with `addPlainTextAttributes()` and
`addTrustedResourceOrigins()`, the `$_x` bypass, and what the defence does not cover.
